...
# Consistency Check: We use LMS_CONFIGS everywhere now
LMS_CONFIGS = {
    "google": {
...
def get_ld(flag):
    ...
    headers = {"Authorization": LD_TOKEN, "LD-API-Version": "beta"}
    try:
...
# --- 2. AUTH & FETCH ---
creds = Credentials.from_service_account_info(json.loads(GOOG_JSON), scopes=["https://spreadsheets.google.com/feeds", "https://www.googleapis.com/auth/drive"])
...
for key, cfg in LMS_CONFIGS.items():
    ...
    # Check if at least one App ID is enabled in this flag
    ...
    for r in rows:
        ...
        # District is only "Done" if it's in LD AND the app gate is open
...
<head>
    ...
    <style>
        body {{ font-family: -apple-system, sans-serif; background: #f4f7f9; padding: 40px; color: #202124; line-height: 1.5; }}
        ...
        .bar div {{ height: 100%; transition: width 1s ease-in-out; }}
        ...
        .sum-count {{ background: #f1f3f4; padding: 2px 12px; border-radius: 12px; font-size: 0.85em; color: #5f6368; }}
        ...
        th {{ color: #5f6368; font-size: 0.8em; text-transform: uppercase; }}
